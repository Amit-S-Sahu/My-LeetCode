class Solution {
    ...
    private boolean canMake(String recipe) {
        if (visited.containsKey(recipe)) return visited.get(recipe) == 1;

        if (availableSupplies.contains(recipe)) return true;

        if (!recipeToIngredients.containsKey(recipe)) return false;
